...
def analyze_injury_with_gemini(
    ...
    gemini_input = build_gemini_injury_input(
        simulation_result
    )
    ...
    # Pydantic schema가 지정되었으므로 parsed 결과를 우선 사용
    assessment = response.parsed

    # 혹시 SDK/version 차이로 parsed가 없는 경우 fallback
    if assessment is None:
        assessment = InjuryAssessment.model_validate_json(
            response.text
        )

    # Gemini가 존재하지 않는 feature를 근거로 삼지 않았는지 검증
    validate_evidence_paths(
        assessment,
        gemini_input,
    )

    # Pydantic 객체 → 일반 Python dict
    result = assessment.model_dump(mode="json")


    # ============================================================
    # Mechanical findings의 evidence path → 실제 값
    # ============================================================
    ...
    # ============================================================
    # Possible injuries의 evidence path → 실제 값
    # ============================================================
    ...
    # AI가 새로 작성하지 않고 원본 limitations를 그대로 복사
